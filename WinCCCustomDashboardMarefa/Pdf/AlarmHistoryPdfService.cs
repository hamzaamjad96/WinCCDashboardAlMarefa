...
public class AlarmHistoryPdfService
{
    public byte[] Generate(List<AlarmEvent> alarms)
    {
        ...
        foreach (var alarm in alarms)
        {
            ...
            row.Cells[0]
                .AddParagraph(index.ToString());

            row.Cells[1]
                .AddParagraph(alarm.AlarmName);

            row.Cells[2]
                .AddParagraph(
                    alarm.RaiseTime.ToString(
            ...
            row.Cells[4]
                .AddParagraph(alarm.SourceServer);

            row.Cells[5]
                .AddParagraph(
                    alarm.SyncedAt.ToString(
                        "yyyy-MM-dd HH:mm:ss.fff"));
